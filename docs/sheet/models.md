...
| `text` | 文章內容（**必填**） |
| `title` | 文章標題，顯示在最上面 |
| `backgroundImg` | **文章開頭的一張插圖**，跟著內文一起捲 |

`missionId` 跟其他頁一樣，留空就是沿用上一關。

::: tip backgroundImg 在這裡不是背景
別的頁面填 `backgroundImg` 是鋪滿整頁的底圖，`Article` 不是——
它是文章開頭的一張圖。**長文要讀得下去，底圖會把對比讓給美術。**
:::
...
::: tip 要放圖請用 backgroundImg
Markdown 的插圖語法不支援。`Article` 的圖填在 `backgroundImg` 欄位。
:::
